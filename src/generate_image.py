...
def generate_image(prompt_text, config, iteration):
    ...
    # Set random seed
    workflow["25"]["inputs"]["noise_seed"] = generate_random_seed()
    ...
    try:
        ...
        # Save the image from the SaveImage node (node 9 in our workflow)
        if '9' in images and images['9']:
            output_path = os.path.join(
                config["comfyui"]["output_dir"],
                f"iteration_{iteration}.png"
            )
            save_image_data(images['9'][0], output_path)
            return output_path
        else:
